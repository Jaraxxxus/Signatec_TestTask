...
void UIManager::run() {
    while (monitor.running.load()) {
        ...
        monitor.cv.wait(lock, [&] { return monitor.updated.load() || !monitor.running.load(); });
        ...
        for (const auto& [key, info] : monitor.proc_data) {
            ...
            if (!info.alive) {
                ...
                std::cout << "[PID " << pid << "] " << name
                          << " CPU: " << std::fixed << std::setprecision(2) << info.cpu_percent << "%"
        ...
        for (const auto& name : monitor.missing_processes) {
            if (alive_names.count(name) == 0 && !monitor.process_missing_displayed[name]) {
