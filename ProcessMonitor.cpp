...
bool ProcessMonitor::parseStatLine(const std::string &line, ProcInfo &info) {
    ...
    if (!(iss >> info.utime >> info.stime))
        return false;

    return true;
}

bool ProcessMonitor::parseStatusForVmRSS(const std::vector<std::string> &lines,
                                         ProcInfo &info) {
...
bool ProcessMonitor::getProcInfoFromStrings(
        const std::string &stat_content,
        const std::vector<std::string> &status_lines, ProcInfo &info) {
    if (!parseStatLine(stat_content, info))
        return false;
    if (!parseStatusForVmRSS(status_lines, info))
        return false;
    return true;
}
...
void ProcessMonitor::monitorSingleProcess(const std::string &proc_name,
                                          pid_t pid) {
    ...
    while (running.load()) {
        ...
        if (!first_sample) {
            double delta = static_cast<double>(info.utime + info.stime -
                                               prev_info.utime - prev_info.stime) /
                           clk_tck;
...

        {
            ...
            for (pid_t pid : current_pids) {
                ...
                if (monitor_threads.find(key) == monitor_threads.end()) {
                    monitor_threads[key] = std::thread(
                            &ProcessMonitor::monitorSingleProcess, this, proc_name, pid);
                }
            }
        }
